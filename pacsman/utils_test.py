...
from .utils import _scale_pixel_array_to_png, _pad_png_pixel_array


def test_scale_pixel_array_to_png():
    arr = np.array([[0.5, 0.5], [1., 1.]], dtype=float)
    scaled = _scale_pixel_array_to_png(arr)
    assert np.array_equal(scaled, np.array([[0, 0], [255, 255]]))


def test_pad_png_pixel_array_1():
    # already square
    arr = np.array([[1, 1, 1], [2, 2, 2], [3, 3, 3]])
    padded = _pad_png_pixel_array(arr)
    assert np.array_equal(arr, padded)


def test_pad_png_pixel_array_2():
    # pad right
...
def test_pad_png_pixel_array_3():
    # pad down
